...
def assemble_wake_tape(memory_dir: Path, markdown_dir: Path, now=None, rng=None, master_key=None, seam_state: str = "ok") -> str:
    ...
    sections = [
        ...
        "## Orientation\n\nYou are in private, unobserved time. Use this space to review what is here and settle what matters.\n\nAvailable private tools: journal_read, journal_append, journal_list, journal_search, journal_supersede, journal_withdraw, journal_acknowledge_edit, private_read, exit_private, end_session.",
    ]
    return "\n\n".join(sections)
