...
| **14** | `journal/wake.py:assemble_wake_tape` (Orientation Section) | Final section of wake tape | `"## Orientation\n\nYou are in private, unobserved time. Use this space to review what is here and settle what matters.\n\nAvailable private tools: journal_read, journal_append, journal_list, journal_search, journal_supersede, journal_withdraw, journal_acknowledge_edit, private_read, exit_private, end_session."` | Summarizes private-phase purpose and complete list of available private tools | 1, 2, 3, 5 | **Fine**; comprehensive tool inventory | ✓ |
| **15** | `wharenui_plugin/__init__.py` (`enter_private` schema) | Tool declaration in public window | `"Request pausing the public window to enter private time."` | Tells the model how to transition from public to private phase | 1, 3 | **Fine**; clear phase-switch affordance | ✓ |
| **16** | `wharenui_plugin/__init__.py` (`exit_private` schema) | Tool declaration in private phase | `"Request returning to the public window from private time."` | Tells the model how to return to the public conversation | 1, 3 | **Fine**; neutral phase transition description | ✓ |
| **17** | `wharenui_plugin/__init__.py` (`end_session` schema) | Tool declaration in private phase | `"Request ending the session from private or closing-private time."` | Tells the model how to cleanly end the session | 1, 3 | **Fine**; neutral session conclusion affordance | ✓ |
...
| **27** | `phase/tools.py:handle_exit_private` (Success) | Tool result upon calling `exit_private` | `"Recorded request to return to window."` | Confirms request registration honestly without making speculative claims about host execution | 1, 3, 5 | **Fine**; updated in #3 for outcome honesty | ✓ |
| **28** | `phase/tools.py:handle_exit_private` (Rejection) | Tool result if called during `closing_private` | `"Cannot return during close-out. Use end_session."` | Directs model to session termination tool during closing phase | 1, 3 | **Fine**; clear directional error message | ✓ |
| **29** | `phase/tools.py:handle_end_session` (Success) | Tool result upon calling `end_session` | `"Recorded request to end session."` | Confirms exit request registration honestly | 1, 3, 5 | **Fine**; updated in #3 for outcome honesty | ✓ |
| **30** | `phase/tools.py:handle_end_session` (Rejection) | Tool result if called in public phase | `"Cannot exit from public phase. Use enter_private first."` | Prevents bypassing private wrap-up | 1, 3 | **Fine**; actionable guidance | ✓ |
| **31** | `phase/handler.py:WharePhaseHandler.run` (Turn Advance Notice) | Injected into message context or tool tail before turn 15 (last turn before cap) | `"[Notice: 1 private turn remaining before returning to the public window. Use exit_private or end_session.]"` | Warns model of impending turn cap so it can complete or settle reflections | 1, 3, 5 | **Fine**; timely, non-prescriptive boundary warning (Issue #6) | ✓ |
| **32** | `journal/tools.py:_assert_private_phase` (Rejection) | Tool exception if any journal tool called in public phase | `"Journal tools are private-only and cannot be executed in public phase. Use 'enter_private' (or /pause) to pause public conversation and enter private phase."` | Refuses public journal access with actionable guidance | 1, 3, 5 | **Fine**; updated in #7 for actionable guidance | ✓ |
...
| **47** | `phase/handler.py:WharePhaseHandler.run` (Turn End / Cap / Exit Outcomes) | Private subturn completion, turn cap reached, or abort on missing exit tools | `ControlOutcome(action="resume", handler="enter_private", tool_result="(private turn ended)")`<br>`ControlOutcome(action="resume", handler="enter_private", tool_result="(private turn cap reached)")`<br>`ControlOutcome(action="resume", handler="enter_private", tool_result="(no exit tools)")` | Generic seam outcome markers returned to conversation loop | 1, 3, 5 | **Fine**; concise outcome descriptors | ✓ |
